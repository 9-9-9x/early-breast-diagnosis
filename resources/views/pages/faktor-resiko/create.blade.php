...
@section('content')
    ...
    @php
        ...
        $radioLabelClasses =
            'flex items-center justify-center w-14 h-14 border border-black rounded-lg cursor-pointer transition-colors';
        ...
        // BMI info passed from controller
        $obesitasValue = isset($isObese) && $isObese ? '1' : '0';
        $bmiDisplay = isset($bmi) ? number_format($bmi, 1) : null;
    @endphp
        ...
        <form method="POST" action="{{ route('faktor-risiko.store') }}" id="faktor-risiko-form" class="w-full max-w-7xl">
                    ...
                    <div class="p-6 sm:p-8" x-data="{ kb_hormonal: '{{ old('kb_hormonal', '0') }}' }"> <!-- Alpine x-data -->
                        ...
                        <div class="space-y-4">
                            @foreach (array_slice($questions_step2, 0, 3) as $question)
                                <div
                                    class="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 py-4 border-b last:border-b-0">
                                    <p class="text-xl md:text-2xl font-medium text-black">{{ $question['label'] }}
                                        {!! $requiredMark !!}</p>
                                    <div class="flex items-center gap-x-8 w-full md:w-auto">
                                        <div class="w-1/2 md:w-14"><input type="radio" name="{{ $question['name'] }}"
                                                id="{{ $question['name'] }}_yes" value="1" class="hidden peer"
                                                @checked(old($question['name']) == '1')><label for="{{ $question['name'] }}_yes"
                                                class="{{ $radioLabelClasses }} {{ $radioPeerCheckedClasses }}"><span
                                                    class="hidden peer-checked:block text-3xl">✅</span></label></div>
                                        <div class="w-1/2 md:w-14"><input type="radio" name="{{ $question['name'] }}"
                                                id="{{ $question['name'] }}_no" value="0" class="hidden peer"
                                                @checked(old($question['name']) == '0')><label for="{{ $question['name'] }}_no"
                                                class="{{ $radioLabelClasses }} {{ $radioPeerCheckedClasses }}"><span
                                                    class="hidden peer-checked:block text-3xl">✅</span></label></div>
                                    </div>
                                </div>
                            ...
                            <div class="border-b"> <!-- Seksi KB Hormonal -->
                                ...
                                <div x-transition class="pl-4 md:pl-8 py-4 space-y-4">
                                    <!-- HAPUS x-show="kb_hormonal === '1'" -->
                                    <div
                                        class="flex flex-col md:flex-row items-start md:items-center justify-between gap-4 py-4 border-t">
                                        <p class="text-xl md:text-2xl font-medium text-black">Pil > 5 Tahun</p>
                                        <!-- Label -->
                                        <div class="flex items-center gap-x-8 w-full md:w-auto">
                                            <!-- Radio "Ya" untuk Pil > 5 Tahun -->
                                            <div class="w-1/2 md:w-14"><input type="radio" name="pil_kb_lebih_5_tahun"
                                                    id="pil_kb_lebih_5_tahun_yes" value="1" class="hidden peer"
                                                    @checked(old('pil_kb_lebih_5_tahun') == '1')><label for="pil_kb_lebih_5_tahun_yes"
                                                    class="{{ $radioLabelClasses }} {{ $radioPeerCheckedClasses }}"><span
                                                        class="hidden peer-checked:block text-3xl">✅</span></label></div>
                                            <!-- Radio "Tidak" untuk Pil > 5 Tahun -->
                                            <div class="w-1/2 md:w-14"><input type="radio" name="pil_kb_lebih_5_tahun"
                                                    id="pil_kb_lebih_5_tahun_no" value="0" class="hidden peer"
                                                    @checked(old('pil_kb_lebih_5_tahun') == '0')><label for="pil_kb_lebih_5_tahun_no"
                                                    class="{{ $radioLabelClasses }} {{ $radioPeerCheckedClasses }}"><span
                                                        class="hidden peer-checked:block text-3xl">✅</span></label></div>
                                        </div>
                                    </div>
                                    <div
                                        ...
                                        <div class="flex items-center gap-x-8 w-full md:w-auto">
                                            ...
                                            <div class="w-1/2 md:w-14"><input type="radio"
                                                    name="suntik_kb_lebih_5_tahun" id="suntik_kb_lebih_5_tahun_yes"
                                                    value="1" class="hidden peer" @checked(old('suntik_kb_lebih_5_tahun') == '1')><label
                                                    for="suntik_kb_lebih_5_tahun_yes"
                                                    class="{{ $radioLabelClasses }} {{ $radioPeerCheckedClasses }}"><span
                                                        class="hidden peer-checked:block text-3xl">✅</span></label></div>
                                            <!-- Radio "Tidak" untuk Suntik > 5 Tahun -->
                                            <div class="w-1/2 md:w-14"><input type="radio"
                                                    name="suntik_kb_lebih_5_tahun" id="suntik_kb_lebih_5_tahun_no"
                                                    value="0" class="hidden peer" @checked(old('suntik_kb_lebih_5_tahun') == '0')><label
                                                    for="suntik_kb_lebih_5_tahun_no"
                                                    class="{{ $radioLabelClasses }} {{ $radioPeerCheckedClasses }}"><span
                                                        class="hidden peer-checked:block text-3xl">✅</span></label></div>
                                        </div>
                                    </div>
            ...
            <div class="w-full mx-auto flex flex-col sm:flex-row justify-end items-center gap-4 px-4 sm:px-0">
                <button type="button" id="prev-btn"
                    class="hidden w-full sm:w-48 h-16 rounded-xl border border-[#3e7b27] text-black font-semibold text-2xl hover:bg-gray-100 transition shadow-md">
                    KEMBALI
                </button>
                <button type="button" id="next-btn"
                    class="w-full sm:w-48 h-16 rounded-xl bg-[#3e7b27] text-white font-semibold text-2xl hover:bg-opacity-90 transition shadow-md">
                    LANJUT
                </button>
                <button type="submit" id="submit-btn"
                    class="hidden w-full sm:w-48 h-16 rounded-xl bg-[#3e7b27] text-white font-semibold text-2xl hover:bg-opacity-90 transition shadow-md">
                    SIMPAN
                </button>
            </div>
        </form>
