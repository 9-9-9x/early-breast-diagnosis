...
@section('content')
    ...
    @php
        ...
        $inputClasses =
            'mt-2 block w-full rounded-lg border-gray-400 shadow-sm focus:ring-[#3e7b27] focus:border-[#3e7b27] h-14 px-4 text-xl';
                ...
                <div class="p-6 sm:p-8">
                    <div class="grid grid-cols-1 lg:grid-cols-2 gap-x-12 gap-y-8">
                        ...
                        <div class="space-y-8">
                            ...
                            <div>
                                <label for="jumlah_anak" class="{{ $labelClasses }}">Jumlah Anak Kandung
                                    {!! $requiredMark !!}</label>
                                <input type="number" id="jumlah_anak" name="jumlah_anak" value="{{ old('jumlah_anak') }}"
                                    class="{{ $inputClasses }}" placeholder="Contoh: 2">
                            </div>
                        </div>
                        ...
                        <div class="space-y-8">
                            <div>
                                <label for="telepon" class="{{ $labelClasses }}">Nomor Telepon
                                    {!! $requiredMark !!}</label>
                                <input type="tel" id="telepon" name="telepon" value="{{ old('telepon') }}"
                                    class="{{ $inputClasses }}" placeholder="081234567890">
                            </div>

                            <div class="grid grid-cols-1 sm:grid-cols-5 gap-4">
                                <div class="sm:col-span-3">
                                    <label for="alamat" class="{{ $labelClasses }}">Alamat
                                        {!! $requiredMark !!}</label>
                                    <input type="text" id="alamat" name="alamat" value="{{ old('alamat') }}"
                                        class="{{ $inputClasses }}" placeholder="Nama jalan / dusun">
                                </div>
                                <div>
                                    <label for="rt" class="{{ $labelClasses }}">RT {!! $requiredMark !!}</label>
                                    <input type="text" id="rt" name="rt" value="{{ old('rt') }}"
                                        class="{{ $inputClasses }}" placeholder="001">
                                </div>
                                <div>
                                    <label for="rw" class="{{ $labelClasses }}">RW {!! $requiredMark !!}</label>
                                    <input type="text" id="rw" name="rw" value="{{ old('rw') }}"
                                        class="{{ $inputClasses }}" placeholder="002">
                                </div>
                            </div>

                            <div>
                                <label for="desa" class="{{ $labelClasses }}">Desa/Kelurahan
                                    {!! $requiredMark !!}</label>
                                <div class="relative">
                                    <select id="desa" name="desa" class="{{ $inputClasses }} appearance-none">
                                        <option value="" disabled selected>Pilih desa/kelurahan</option>
                                        <option value="pakusari" @selected(old('desa') == 'pakusari')>Pakusari</option>
                                        <option value="patemon" @selected(old('desa') == 'patemon')>Patemon</option>
                                        <option value="subo" @selected(old('desa') == 'subo')>Subo</option>
                            ...
                            <div>
                                <label for="pendidikan" class="{{ $labelClasses }}">Pendidikan Terakhir
                                    {!! $requiredMark !!}</label>
                                <div class="relative">
                            ...
                            <div>
                                <label for="pekerjaan_pasien" class="{{ $labelClasses }}">Pekerjaan Pasien
                                    {!! $requiredMark !!}</label>
                                <div class="relative">
                                    <select id="pekerjaan_pasien" name="pekerjaan_pasien"
                                        class="{{ $inputClasses }} appearance-none">
                                        <option value="" disabled selected>Pilih pekerjaan pasien</option>
                                        <option value="Ibu Rumah Tangga" @selected(old('pekerjaan_pasien') == 'Ibu Rumah Tangga')>Ibu Rumah Tangga
                                        </option>
                                        <option value="Wiraswasta" @selected(old('pekerjaan_pasien') == 'Wiraswasta')>Wiraswasta</option>
                                        <option value="Pegawai Swasta" @selected(old('pekerjaan_pasien') == 'Pegawai Swasta')>Pegawai Swasta
                                        </option>
                            ...
                            <div>
                                <label for="pekerjaan_suami" class="{{ $labelClasses }}">Pekerjaan Suami
                                    {!! $requiredMark !!}</label>
                                <div class="relative">
                                    <select id="pekerjaan_suami" name="pekerjaan_suami"
                                        class="{{ $inputClasses }} appearance-none">
                                        ...
                                        <option value="Pegawai Swasta" @selected(old('pekerjaan_suami') == 'Pegawai Swasta')>Pegawai Swasta
                                        </option>
                            ...
                            <div>
                                <label for="status_perkawinan" class="{{ $labelClasses }}">Status Perkawinan
                                    {!! $requiredMark !!}</label>
                                <div class="relative">
